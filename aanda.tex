...
\title{The atomic-to-molecular transition in a colliding flow: \\ \Large{CO and H{\large \Rmnum{1}} observations of the Draco nebula}}
...
\institute{
   Institut d’Astrophysique Spatiale, CNRS, Univ. Paris-Sud, Université Paris-Saclay, bâtiment 121, 91405 Orsay cedex, France \and
%   email: quentin.salome@ias.u-psud.fr \and
   Instituto de Radioastronom\'ia y Astrof\'isica, Universidad Nacional Aut\'onoma de M\'exico, Apdo. Postal 3-72, Morelia 58089, M\'exico \\
   email: q.salome@irya.unam.mx \and
...
\titlerunning{The Draco nebula as seen in CO and H{\rmnum{1}}}
...
\section{Introduction}
   ...
   Most of the mass of the Galactic interstellar medium (ISM) comes from atomic gas as traced in H\rmnum{1}. It is now well established that the neutral hydrogen is thermally bi-stable \citep{Clark_1965,Dickey_2003}, with two phases in pressure equilibrium \citep{Field_1965,Field_1969,Goldsmith_1969, Wolfire_1995a,Wolfire_2003}: the dense cold neutral medium (CNM - $T\sim 50\: K$, $n\sim 50\: cm^{-3}$) bathed in the more diffuse warm neutral medium (WNM - $T\sim 8000\: K$, $n\sim 0.1\: cm^{-3}$). This is the result of a balance between heating and cooling. The structure of the H\rmnum{1} gas is complex with large density variations and is mainly driven by turbulence and thermal instability \citep{Field_1965,Field_1969}.
...
%   Our Galaxy, as all galaxies, is an open and dynamical system. Matter is constantly arriving in the halo and is accreted onto the galactic disc. Part of this gas is the result of the galactic fountain: hot gas is expelled from the disc into the halo due to stellar winds and supernovae then returns to the disc due to gravity \citep{Shapiro_1976,Bregman_1980, Putman_2012}. The intermediate-velocity clouds (IVC) are thought to be the result of the Galactic fountain and are observed at velocities of tens of $km.s^{-1}$.

   The Draco Nebula is one of the best candidates to understand this process and resulting physical conditions. This high Galactic latitude IVC ($v\sim -25\: km\, s^{-1}$) is located above the Galactic plane, at a height $z=370\: pc$ (distance $\sim 600\: pc$). Based on the analysis of H\rmnum{1}-21cm data, it was suggested that Draco has formed following the collision of a cloud entering the disc of the Milky Way \citep{Goerigk_1983}.
...
The sustained pressure in the front also favors the formation of molecules, revealed by the strong $^{12}$CO emission detected \citep{Mebold_1985}. These very specific physical conditions are ideal to study the H\rmnum{1}-to-$H_2$ phase transition in a colliding flow.

   The goal of this paper is to study the dynamics of the molecular and atomic gas at the shock interface of the Draco nebula. To do so, we mapped the interface in $^{12}CO$ (1-0 and 2-1) with the IRAM 30m telescope. These new CO data will be compared with H\rmnum{1}-21cm data from the GHIGLS \citep{Martin_2015} and DHIGLS \citep{Blagrave_2017} surveys.
The comparison of the high resolution Herschel data with H\rmnum{1} and CO data will also enable us to study the presence of CO-dark gas across the interface.
The paper is organized as follows. Section \ref{sec:Obs} presents the CO and H\rmnum{1} data we used for this study. In section \ref{sec:Res} we analyse and compare the CO, H\rmnum{1} and dust emission. Our results are discussed in Sect. \ref{sec:discussion} and summarized in Sect. \ref{sec:conclusion}.% For consistency with our previous study \citep{MAMD_2017b}, we adopt a distance of 600 pc in this paper.
...
\begin{figure*}[h]
  ...
  \caption{\label{Phases_GHIGLS} Maps of the H\rmnum{1} column density in units of $10^{20}\: cm^{-2}$ recovered by the coherent multi-Gaussian decomposition applied on the GHIGLS data. We separated the cold (\emph{left}) and warm (\emph{right}) phases for the IVC (\emph{top}) and the LVC (\emph{bottom}).}
\end{figure*}
...
We thus adjusted the coherent decomposition avoiding to fit noise in the data. The total H\rmnum{1} column density map reconstructed with the Gaussian decomposition of the DHIGLS data for the combination of the LVC and the IVC (hereafter LVC+IVC) is presented in figure \ref{NHI_DHIGLS}. By construction, the limit in the gaussian width $\sigma_{lim}$ is the same for GHIGLS and DHIGLS so we also choosed $\sigma_{lim}=4\: km.s^{-1}$ to quantitatively separate between the cold and warm phases.

\begin{figure*}[h!]
  \centering
  \includegraphics[width=0.8\linewidth,trim=80 35 55 105,clip=true]{Figures/DHIGLS_NHI_tot.png}
  \caption{\label{NHI_DHIGLS} Map of the H\rmnum{1} column density in units of $10^{20}\: cm^{-2}$ recovered by the coherent multi-Gaussian decomposition applied on the DHIGLS data.}
\end{figure*}
...
Contrary to the IVC, the local H\rmnum{1} component shows only little cold neutral medium, including a tail-like structure at the bottom left. However, the WNM is rather homogeneous with column densities of $0.6-1.2\times 10^{20}\: cm^{-2}$. In particular, at the bottom left, the WNM shows a bubble-like feature. The mass of the WNM is $M_{WNM}\sim 1.4\times 10^3\: M_\odot$, of the same order than that of the WNM in the IVC.
From the \emph{Herschel} data presented in \cite{MAMD_2017b}, we derived a total gas mass of $1.8\times 10^4\: M_\odot$, about 4-5 times larger than the total H\rmnum{1} mass $M_{H\rmnum{1}}\sim 4\times 10^3\: M_\odot$. This strengthens the idea that the Draco nebula is mostly composed of molecular gas.
...
%dispersion $\sigma$ and the mean position $v_{r}$ of the Gaussian sample of \textit{DR} is shown figure \ref{fig::heatmap}.
   ...
   The multi-Gaussian decomposition method provides a list of Gaussians. Each Gaussian is characterised by five parameters (its coordinates $\mathnormal{\alpha_i}$,$\mathnormal{\delta_i}$ and its spectral parameters $\mathnormal{A_i}$, $\mathnormal{v_i}$, $\mathnormal{\sigma_i}$).
\textbf{New velocity separation v$_{IVC}<-$13$\: $km.s$^{-1}$, based on heatmap}

   We used these Gaussians to compute maps of the velocity and velocity dispersion. To do so, we first reconstructed the signal in each pixel:
\begin{equation}
...
\begin{figure*}[h]
  ...
  \hspace{3mm}                                
  \includegraphics[page=5,height=7.5cm,trim=140 20 125 70,clip=true]{Figures/GHIGLS_velo.pdf}
  \caption{\label{velo_HI} Maps of the H\rmnum{1} central velocity in $km.s^{-1}$ derived for the CNM from DHIGLS (\emph{top}) and the WNM from GHIGLS (\emph{bottom}). We separated between the intermediate- (\emph{left}) and low-velocity (\emph{right}) components.}
\end{figure*}
...
\begin{figure*}[h!]
  ...
  \caption{\label{Xco} Correlation between the molecular gas column density derived from dust and H\rmnum{1} emission, and the CO integrated intensity. The slope of the best fitting model (blue line) gives the value of the $X_{CO}$ factor. The vertical black dotted line indicates the limit $I_{CO}=2.5\: K.km.s^{-1}$ we used to make the fit.}
\end{figure*}
...
\begin{figure*}[h!]
  ...
  \caption{\label{DRAO-CO} Map of the H\rmnum{1} emission in the IVC from DHIGLS. The black contours show the CO(1-0) emission observed with the IRAM 30m telescope in the region represented by the dashed boxes.}
\end{figure*}

   In order to compare the spatial distribution of CO and H\rmnum{1} emission, we overlaid the CO contours on the column density map of the CNM in the IVC (figure \ref{DRAO-CO}). Along the shock front, CO and H\rmnum{1} are spatially correlated, even though the peak of H\rmnum{1} is located slightly before the peak of CO emission. This result suggests that the shock of the Draco nebula progenitor and the WNM of the Milky Way favoured a phase transition between the atomic CNM and the molecular gas.
Such shift is also observed in Draco1, at the edge of the nose of the nebula. On the opposite, CO and H\rmnum{1} seem to be well mixed in Draco3. We conclude that the nose of the nebula is independant to the shock front.
We used the CO(1-0) intensity and H\rmnum{1}-CNM column density maps to derive a ratio between the molecular and atomic column density in the IVC. As previously shown by \cite{Planck_XXIV_2011,MAMD_2017b}, the Draco nebula is mostly molecular. We found a ratio $N(H_2)/N(H\rmnum{1},CNM)\sim 1.2-1.3$ along the shock front, while this ratio is about 2.0 in Draco1 and 8.9 in Draco3.
...
\end{itemize}
      ...
      \subsubsection{CO-H\rmnum{1} dynamics}

   We now aim to compare the dynamics of the CO and H\rmnum{1} emission. First, we compared the velocity of the CO(1-0) and CO(2-1). As expected both CO lines have rather the same velocity, with a relative difference of $2.8-3.8\%$. We then compared the velocity maps of the CO(1-0) and H\rmnum{1} emission at the resolution of the DHIGLS data. The velocity of the CNM is rather similar to that of CO emission with differences of less than $0.5\: km.s^{-1}$, except in Draco4 where this difference is $\sim 2\: km.s^{-1}$. This indicates that the CNM and the molecular gas are well mixed in all the regions observed with the IRAM 30m.

   We computed position-velocity (PV) diagrams for the H\rmnum{1} and CO(1-0) emission. To do so, we first decreased the spatial and spectral resolution of the CO data to those of the HI data ($1'$ in space and $0.8\: km.s^{-1}$ in velocity). For the H\rmnum{1} emission, we only considered the cold phase in the IVC. To compute a PV diagram, we stacked all the pixels in the direction orthogonal to the "slit" we consider.

   For both emission, we focused on the region where CO has been observed. We finally stacked all the pixels in the direction orthogonal to the "slit".% The resulting PV diagrams are shown in figure \ref{PV-diag}.
In particular, we focused on the region Draco4-5-6. 

along slit centred on the shock front and oriented at $\pm 45$\degree.

   For region Draco2, we combuted PV diagrams along four slits: the vertical and horizontal directions, and the $\pm 45$\degree directions. The four PV diagrams show a good correlation between CO and H\rmnum{1}. In the PV diagram, we also seen a high velocity feature in CO. This feature was identified as the bright structure centred at $\alpha=16^h 50^m 56^s.4$, $\delta=+$60:54:18.2.
...
%%  \centering
%  \hspace{5mm}
%  \includegraphics[page=2,height=7cm,trim=180 85 185 80,clip=true]{Figures/CO10_intensity.pdf}
%  \hspace{3mm}
%  \includegraphics[page=4,height=7cm,trim=205 10 200 40,clip=true]{Figures/PV_diagram_CO_vertical.pdf}
%  \includegraphics[page=4,height=7cm,trim=205 10 200 40,clip=true]{Figures/PV_diagram_HI_vertical.pdf} \\
%  \includegraphics[page=4,width=7.7cm,trim=30 100 60 100,clip=true]{Figures/PV_diagram_CO_horizontal.pdf} \\
%  \includegraphics[page=4,width=7.7cm,trim=30 100 60 100,clip=true]{Figures/PV_diagram_HI_horizontal.pdf}
%  \caption{}
%\end{figure*}
...
%%  \centering
%  \hspace{5mm}
%  \includegraphics[page=5,height=8cm,trim=285 85 295 80,clip=true]{Figures/CO10_intensity.pdf}
%  \hspace{3mm}
%  \includegraphics[page=6,height=8cm,trim=200 10 185 40,clip=true]{Figures/PV_diagram_CO_vertical.pdf}
%  \includegraphics[page=6,height=8cm,trim=200 10 185 40,clip=true]{Figures/PV_diagram_HI_vertical.pdf} \\
%  \includegraphics[page=6,width=6.5cm,trim=30 40 45 40,clip=true]{Figures/PV_diagram_CO_horizontal.pdf} \\
%  \includegraphics[page=6,width=6.5cm,trim=30 40 45 40,clip=true]{Figures/PV_diagram_HI_horizontal.pdf}
%  \caption{}
%\end{figure*}
...
%%  \centering
%  \hspace{5mm}
%  \includegraphics[page=4,height=8cm,trim=320 85 325 80,clip=true]{Figures/CO10_intensity.pdf}
%  \hspace{3mm}
%  \includegraphics[page=5,height=8cm,trim=185 10 170 40,clip=true]{Figures/PV_diagram_CO_vertical.pdf}
%  \includegraphics[page=5,height=8cm,trim=185 10 170 40,clip=true]{Figures/PV_diagram_HI_vertical.pdf} \\
%  \includegraphics[page=5,width=5.6cm,trim=85 40 110 40,clip=true]{Figures/PV_diagram_CO_horizontal.pdf} \\
%  \includegraphics[page=5,width=5.6cm,trim=85 40 110 40,clip=true]{Figures/PV_diagram_HI_horizontal.pdf}
%  \caption{}
%\end{figure*}
...
%%  \centering
%  \hspace{5mm}
%  \includegraphics[page=6,height=8.5cm,trim=335 85 342 80,clip=true]{Figures/CO10_intensity.pdf}
%  \hspace{3mm}
%  \includegraphics[page=7,height=8.5cm,trim=165 10 150 40,clip=true]{Figures/PV_diagram_CO_vertical.pdf}
%  \includegraphics[page=7,height=8.5cm,trim=165 10 150 40,clip=true]{Figures/PV_diagram_HI_vertical.pdf} \\
%  \includegraphics[page=7,width=5.6cm,trim=125 40 150 40,clip=true]{Figures/PV_diagram_CO_horizontal.pdf} \\
%  \includegraphics[page=7,width=5.6cm,trim=125 40 150 40,clip=true]{Figures/PV_diagram_HI_horizontal.pdf}
%  \caption{}
%\end{figure*}
...
%%  \centering
%  \hspace{5mm}
%  \includegraphics[width=9cm,trim=130 295 540 290,clip=true]{Figures/Draco_FRONT_intensity.png}
%  \hspace{3mm}
%  \includegraphics[page=2,height=7cm,trim=233 10 220 40,clip=true]{Figures/PV_diagram_CO_vertical.pdf}
%  \includegraphics[page=2,height=7cm,trim=233 10 220 40,clip=true]{Figures/PV_diagram_HI_vertical.pdf} \\
%  \includegraphics[page=2,width=10cm,trim=30 165 50 160,clip=true]{Figures/PV_diagram_CO_horizontal.pdf} \\
%  \includegraphics[page=2,width=10cm,trim=30 165 50 160,clip=true]{Figures/PV_diagram_HI_horizontal.pdf}
%  \caption{}
%\end{figure*}
...
%
%   3C 285 was detected in CO(1-0) and CO(2-1) with a total molecular gas mass of $\sim 10^{10}\: M_\odot$, meaning a gas fraction of $\sim 2.5\%$. Surprisingly for this kind of source, 3C 285 has a fairly high SFR of $\sim 20\: M_\odot.yr^{-1}$. With a depletion time of less than a Gyr, this source looks like typical star forming galaxies, as shown by its position in a KS-diagram. The simple analytical models that reproduce the CO line profiles are also the ones where the gas is lying in a compact molecular ring/disc of 1-2 kpc. So star formation may proceed in this very compact region hidden inside a larger dust-lane seen in the optical image and which crosses the entire galaxy. Note that 3C 285 has a much more massive molecular gas reservoir than NGC 541, standing in a disc of about the same size. So the molecular gas density must be higher which could explain why the SFE ($1/t_{dep}$) is higher in this object.
%
...
\bibliography{Biblio}%,Biblio_Antoine}
