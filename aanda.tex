...
\begin{figure*}[h]
  \centering
  \includegraphics[page=3,height=6.5cm,trim=75 40 55 105,clip=true]{Figures/DHIGLS_NHI.pdf}
  \hspace{3mm}
  \includegraphics[page=6,height=6.5cm,trim=75 40 55 105,clip=true]{Figures/DHIGLS_NHI.pdf}
  \caption{\label{NHI_DHIGLS} Map of the total H\rmnum{1} column density in units of $10^{20}\: cm^{-2}$ recovered by the coherent multi-Gaussian decomposition applied on the DHIGLS data, for the cold (\emph{left}) and warm (\emph{right}) phases.}
\end{figure*}
...
\textbf{    The velocity gradient observed in H\rmnum{1} at the interface  with the Rayleigh-Taylor instability strengthens the scenario of a shock between the cloud at the origin of Draco and the warm neutral 
...
\begin{table}[h]
  ...
  \end{tabular}
  \caption{\label{table:Xco} Values of the $X_{CO}$ factor estimated with a double linear regression for $I_{CO}\leq 2.5\: K.km.s^{-1}$ and $I_{CO}>2.5\: K.km.s^{-1}$. For Draco3, the integrated intensity is always greater than $2.5\: K.km.s^{-1}$ therefore we only made a single regression.}
\end{table}
...
\begin{figure*}[h!]
  ...
  \caption{\label{Xco} Correlation between the molecular gas column density derived from dust and H\rmnum{1} emission, and the CO integrated intensity. The black and blue lines are the best fitting models for the two regimes separated by the vertical black dotted line ($I_{CO}=2.5\: K.km.s^{-1}$).}
\end{figure*}
